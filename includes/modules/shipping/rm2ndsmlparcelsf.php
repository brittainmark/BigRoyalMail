...
class rm2ndsmlparcelsf {
	var $code, $title, $description, $enabled, $num_zones ;


	// class constructor
	function rm2ndsmlparcelsf() {
...
echo "in quote";
		...
		if ($dest_zone == 0) {
			...
			$shipping = -1;
			//check to see if medium or Parcelforce is set
			$products = $_SESSION['cart']->get_products();
			$num_products=sizeof($products);
			echo "<br> Num products = ".$num_products;
			foreach ($products as $product) {
				echo "<br> in product loop product model= ".$product['model'];
				echo "<br>product attributes size =".sizeof($product['attributes']);
				echo isset($product['attributes'])?"<br>Attributes set":"<br>Attributes Not Set";
				echo is_array($product['attributes'])?"<br>Is array":"<br> Not an Array";
				echo  $product['attributes']?"<br> Attributes value = true":"<br> Attributes value = false";

				ECHO "procust att value = ".$product['attributes']["value"];

				// Check attributes name
				if (isset($product['attributes']) && is_array($product['attributes'])) {
					foreach ($product['attributes'] as $option => $value) {
						echo "<br> in attributes loop value = ".$value;
						$attributes = "SELECT  poval.products_options_values_name
								FROM " . TABLE_PRODUCTS_OPTIONS_VALUES . " poval
										WHERE  poval.products_options_values_id = :optionsValuesID
										AND poval.language_id = :languageID " ;

						$attributes = $db->bindVars($attributes, ':optionsValuesID', $value, 'integer');
						$attributes = $db->bindVars($attributes, ':languageID', 1, 'integer');
						$attributes_values = $db->Execute($attributes);
						$newvalue = $attributes_values->fields['products_options_values_name'];
						echo "<br> in attributes loop value = ".$newvalue;
						if ($newvalue == "Royal Mail Medium Parcel" || $newvalue == "Parcel Force"){
			...
			$order->products[$index]['attributes']['value'];
	...
	function install() {
		...
		$db->Execute("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) VALUES ('Sort Order', 'MODULE_SHIPPING_" . $module . "_SORT_ORDER', '130', 'Sort order of display.', '6', '0', now())");
